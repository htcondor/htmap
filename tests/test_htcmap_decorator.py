...
def test_name_is_given_name(doubler):
    n = 'joe'
    mapper = htmap.htmap(name = n)(doubler)

    assert mapper.name == n


def test_htmap_is_idempotent_if_no_name_given(mapped_doubler):
...
def test_repeated_htmap_has_same_func_if_name_given(mapped_doubler):
    n = 'joe'
    mapper = htmap.htmap(name = n)(mapped_doubler)
