...
def test_one_reserved_kwarg_raises():
    with pytest.raises(htmap.exceptions.ReservedOptionKeyword) as exc_info:
        htmap.MapOptions(transfer_input_files="foobar",)
...
def test_two_reserved_kwarg_raises():
    with pytest.raises(htmap.exceptions.ReservedOptionKeyword) as exc_info:
        htmap.MapOptions(
            transfer_input_files="foobar", jobbatchname="pink",
        )
...
def test_reserved_kwargs_are_case_insensitive():
    with pytest.raises(htmap.exceptions.ReservedOptionKeyword):
        htmap.MapOptions(JobBatchName="pink",)


def test_request_memory_for_str():
    opts = htmap.MapOptions(request_memory="150MB")

    assert opts["request_memory"] == "150MB"


def test_request_disk_for_str():
    opts = htmap.MapOptions(request_disk="1.2GB")
...
def test_list_of_list_of_path_input_files(tmp_path):
    ...
    expected = [
        {
            "component": "0",
            "extra_input_files": f"{Path('foo.txt').absolute().as_posix()}, {Path('foo2.txt').absolute().as_posix()}",
        },
        {
            "component": "1",
            "extra_input_files": f"{Path('bar.txt').absolute().as_posix()}, {Path('bar2.txt').absolute().as_posix()}",
        },
        {
            "component": "2",
            "extra_input_files": f"{Path('buz.txt').absolute().as_posix()}, {Path('buz2.txt').absolute().as_posix()}",
        },
    ]
